angular.module('smartmeetApp')
    .service('dbService', function ($q, $timeout, $rootScope, APIServiceFactory, ENV, Service, cordovaService) {
        ...
        this.init = function () {
            ...
            openRequest.onupgradeneeded = function (e) {
                ...
                if (!thisDb.objectStoreNames.contains("Comments")) {
                    objectStore = thisDb.createObjectStore("Comments", {
                        autoIncrement: true
                    });
                }
        ...
        this.addMeetingById = function (data) {
            ...
            var objectStore = db.transaction("Meetings", "readwrite").objectStore("Meetings");
            $.each(data, function (key, value) {
                var request = objectStore.put(JSON.stringify(data[key]), data[key].SMLocatorOWSTEXT);
                request.onsuccess = function (event) {
                    //console.log("Data Insertion success");
                    promises.push(request);
                };
        ...
        this.saveOfflineNote = function (meetingId, atendeeList, docUrl, comments, sessionId, isShared) {
            ...
            var docname = docUrl.substr(docUrl.lastIndexOf('/') + 1).split('.')[0];
            var key = meetingId + '_' + sessionId + '_' + docname;
            var offlineNote = {
                "meetingId": meetingId,
            ...
            //            var req = objectStore.clear();
            var request = objectStore.add(JSON.stringify(offlineNote), key);
            request.onsuccess = function (event) {
                console.log("Success");
            }
            request.onerror = function (e) {
        ...
        this.saveComments = function (meeting, docUrl, comments, sessionId, isShared, key, meetingid, dayid, agendaid) {
            console.log(meeting);
            var docname = docUrl.substr(docUrl.lastIndexOf('/') + 1).split('.')[0];
            var key = meeting.SMLocatorOWSTEXT + '_' + sessionId + '_' + docname;
            var offlineNote = {
                "meetingId": meeting.SMLocatorOWSTEXT,
                "meetingName": meeting.SMNameOWSTEXT,
                "docUrl": docUrl,
                "comments": comments,
                "sessionId": sessionId,
                "isShared": isShared,
            ...
            console.log(offlineNote);
            var objectStore = db.transaction("Comments", "readwrite").objectStore("Comments");
            //            var req = objectStore.clear();
            var request = objectStore.put(JSON.stringify(offlineNote), key);
            request.onsuccess = function (event) {
                console.log("Success");
            }
            request.onerror = function (e) {
        ...
        this.clearOfflineNote = function (key) {
            var objectStore = db.transaction("OfflineNotes", "readwrite").objectStore("OfflineNotes");
