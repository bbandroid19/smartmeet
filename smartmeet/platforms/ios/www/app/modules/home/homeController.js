    angular.module('smartmeetApp')
        .controller('homeController', ['$scope', '$state', 'Service', 'ENV', '$rootScope', '$stateParams', '$timeout', '$sce', 'cordovaService', '$window', 'dataService', 'dbService', '$compile', '$q', '$interval', 'ngProgressFactory', function ($scope, $state, Service, ENV, $rootScope, $stateParams, $timeout, $sce, cordovaService, $window, dataService, dbService, $compile, $q, $interval, ngProgressFactory) {
            $scope.init = function () {
                //                $rootScope.saveinBackground = false;
                $scope.showModal = false;
                $scope.selected = {};
                $scope.timezoneOn = false;
                $scope.timeZoneText = "Local Time Zone"
                ...
                if (LocalFileSystem) {
                    window.requestFileSystem(LocalFileSystem.PERSISTENT, 0, function (fileSystem) {
                        ...
                        console.log("Error in retreiving native location");
                        console.log("fails!");
                    });
                }
                $scope.checkOfflineNotes();
            };
            $scope.changeTimezone = function () {
                $scope.timezoneOn = !$scope.timezoneOn
                if ($scope.timezoneOn) {
                    $scope.timeZoneText = "Meeting Time Zone"
                    $scope.meetingTimezone = $scope.meeting[$scope.meetingIndex].SMTZNameOWSTEXT;
                    console.log($scope.meetingTimezone);
                } else {
                    $scope.timeZoneText = "Local Time Zone"
                    $scope.meetingTimezone = ""
                }

            };
            $scope.loadCommentData = function () {
                dbService.getComments().then(function (result) {
                    console.log(result);
                    $scope.commentsEntry = result;
            ...
            $scope.getMyUserId = function () {
                if ($scope.Attendees) {
                    ...
                    $scope.attendeesList = $scope.Attendees[0].SMAttendees.results;
                    localStorage.setItem("attendeesList", $scope.Attendees[0].SMAttendees.results);
            ...
            $scope.saveDataOffline = function () {
                dbService.addMeetingById($scope.meeting).then(function () {
                    console.log("Data Added");
                });
            };
            $scope.checkOfflineNotes = function () {
                dbService.getOfflineNote().then(function (offlineNote) {
                    console.log(offlineNote);
                    $rootScope.saveinBackground = true;
                    if ($rootScope.online) {
                        for (var i = 0; i < offlineNote.length; i++) {
                            //                        console.log()
                            dbService.clearOfflineNote(offlineNote[i].key);
                        ...
                        //                        $rootScope.saveinBackground = !$rootScope.saveinBackground;
                    }
                                    ...
                                    Service.getAttachments($scope.meeting[$scope.meetingIndex].SMLocatorOWSTEXT, $scope.agenda_arr[$scope.agendaIndex].ID).then(
                                        function (response) {
                                            if (response) {
                                                ...
                                                cordovaService.showCordovaAlert("Document fetch error");
                                                Service.showLoader(false);
                                            }
                                            Service.showLoader(false);
                                        });
            ...
            $scope.loadDocument = function (url, isPresent) {
                ...
                if (isPresent) {
                    ...
                    if ($scope.doc_type === "pdf") {
                        ...
                        $scope.urlContent = $sce.trustAsResourceUrl($scope.fileurl);
            ...
            $scope.toggleNote = function () {
                $scope.savedNote = !$scope.savedNote;
            }
            $scope.loadComments = function () {
                //                $scope.attendeesList = JSON.parse(localStorage.getItem("attendeesList"));
                $scope.savedNote = true;
                $scope.draft = false;
                angular.element('#draftnote').val("");
                //                $scope.draftNote = false;
                var docname = $scope.Siteurl.substr($scope.Siteurl.lastIndexOf('/') + 1).split('.')[0];
                var match = $scope.meeting[$scope.meetingIndex].SMLocatorOWSTEXT + '_' + $scope.sessionId + '_' + docname;
                dbService.getOfflineNote().then(function (data) {
                    if (data) {
                        angular.forEach(data, function (item, index) {
                            if (item.key === match) {
                                $scope.savedNote = false;
                                $scope.draft = true;
                                $scope.draftComment = item.comments;
                            }
                        })
                ...
                if ($rootScope.online) {
                        ...
                        if (commentData) {
                            for (var i = 0; i < commentData.length; i++) {
                                ...
                                if (commentData[i].SMDCFilePath.replace(/%20/g, " ").replace('&amp;wdSmallView', '') === commentUrl) {
                                    var attendeesIndex = $scope.attendeesList.map(function (e) {
                                        return e.Id;
                                    }).indexOf(commentData[i].SMDCommentedById);
                                    if ($scope.isPDF) {
                                        $scope.comments = commentData[i].SMDCComments;

                                    } else {
                                        if (commentData[i].SMDCommentedById === $scope.myId) {
                                            $scope.comments = commentData[i].SMDCComments;
                                        } else if (commentData[i].SMDCShare) {
                                            angular.forEach(commentData[i].SMDCToEmailId.results, function (item, index) {
                                                if (item === $scope.myId) {
                                                    var commentBy = $scope.attendeesList[attendeesIndex].Title;
                                                    $scope.commentsBy += commentBy + ':' + commentData[i].SMDCComments + '\n';
                                                }
                                            })
                                        }
                            ...
                            if (typeof (cordova) != "undefined" && window.plugins) {
                                cordovaService.showCordovaAlert("No Comments Returned ");
                            } else {
                    ...
                    $scope.docnotes = $scope.agenda_arr[$scope.agendaIndex].AttachArray[$scope.docIndex].notes.text;
                    $scope.comments = $scope.docnotes;
                    angular.element('#docnote').val($scope.comments);
                    console.log($scope.agenda_arr[$scope.agendaIndex].AttachArray[$scope.docIndex].notes.text)
            ...
            $scope.savenote = function () {
                if ($scope.savedNote) {
                    ...
                    $scope.agenda_arr[$scope.agendaIndex].AttachArray[$scope.docIndex].notes.text = angular.element('#draftnote').val();

                }
                var docname = $scope.Siteurl.substr($scope.Siteurl.lastIndexOf('/') + 1).split('.')[0];
                var comment_key = $scope.meeting[$scope.meetingIndex].SMLocatorOWSTEXT + '_' + $scope.sessionId + '_' + docname;
                console.log(comment_key);

                dbService.saveComments($scope.meeting[$scope.meetingIndex], $scope.Siteurl, $scope.agenda_arr[$scope.agendaIndex].AttachArray[$scope.docIndex].notes.text, $scope.sessionId, false, comment_key, $scope.meetingIndex,
                    $scope.dayIndex,
                    $scope.agendaIndex);
                $scope.loadCommentData();
                if ($rootScope.online) {
                    ...
                    $scope.draftComment = $scope.agenda_arr[$scope.agendaIndex].AttachArray[$scope.docIndex].notes.text;
                    $scope.draft = true;
            ...
            $scope.fileDownload = function (index) {
                ...
                $rootScope.progressText = "Loading File";
            ...
            $scope.toggleComments = function () {
                $scope.loadCommentData();
                document.getElementById("commentsDropdown").classList.toggle("show");
            };
            $scope.openComment = function (comment) {
                $scope.meetingIndex = comment.meetingid;
                $scope.dayIndex = comment.dayid;
                $scope.agendaIndex = comment.agendaid;
                $scope.changeHomeData();
            };
            $scope.checkCommentedDoc = function (item) {
                var ret = false;
                var key = $scope.meeting[$scope.meetingIndex].SMLocatorOWSTEXT + '_' + $scope.sessionId + '_' + item.FileName.split('.')[0];
                angular.forEach($scope.commentsEntry, function (item, index) {
                    if (item.key === key) {
                        ret = true;
                    }
                })
            ...
            $scope.disableText = function () {

                if (!$scope.draftComment || $scope.draftComment === "") {
                    $scope.draftComment = $scope.comments;
